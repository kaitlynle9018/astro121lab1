...
def compute_power_spectrum(data, sample_rate, method='fft', freq_oversampling=1):
    ...
    if method == 'fft':
        ...
        # 1. Define Time Vector (0 to total time)
        times = np.arange(N) / sample_rate
        
        # 2. Define Frequency Vector
        # We span -fs/2 to +fs/2. Oversampling adds more points in between.
        f_min = -sample_rate / 2
        f_max = sample_rate / 2
        num_freqs = N * freq_oversampling
        freqs = np.linspace(f_min, f_max, num_freqs)
        
        # 3. Manual DFT (Matrix Multiplication)
        # Create a grid of exponents: exponent = -2j * pi * f * t
        # We use outer product to create a (Freqs x Time) matrix
        exponent = -2j * np.pi * np.outer(freqs, times)
        basis = np.exp(exponent)
        
        # Dot product: Sum over time for each frequency
        spectrum = basis @ data
